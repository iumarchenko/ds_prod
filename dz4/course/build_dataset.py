...
import pandas as pd
import warnings
from util.utils import time_format
from util.conf import CHURNED_END_DATE,CHURNED_START_DATE,INTER_LIST
import logging
...
def build_dataset_raw(churned_start_date='2019-01-01',
    ...
    LOGGER.debug('Start reading csv files')
    ...
    LOGGER.debug('Run time (reading csv files): {}'.format(time_format(time.time() - start_t)))
    # -----------------------------------------------------------------------------------------------------
    print('NO dealing with outliers, missing values and categorical features...')
    LOGGER.debug('NO dealing with outliers, missing values and categorical features...')
    # -----------------------------------------------------------------------------------------------------
    ...
    for df in [payments, reports, abusers, logins, pings, sessions, shop]:
        ...
        LOGGER.debug('Processing with features:', features)
        for feature in features:
            for i, inter in enumerate(inter_list):
                inter_df = \
                data.loc[data['day_num_before_churn'].between(inter[0], inter[1], inclusive=True)].groupby('user_id')[
                    feature].mean().reset_index().rename(index=str, columns={feature: feature + '_{}'.format(i + 1)})
        ...
        LOGGER.debug('Run time (calculating features): {}'.format(time_format(time.time() - start_t)))
    ...
    print('Dataset is successfully built and saved to {}, run time "build_dataset_raw": {}'.format(dataset_path,
                                                                                                   time_format(
                                                                                                       time.time() - start_t)))
    LOGGER.debug('Dataset is successfully built and saved to {}, run time "build_dataset_raw": {}'.format(dataset_path,
                                                                                                   time_format(
                                                                                                       time.time() - start_t)))


LOGGER.debug('Build train dataset')
build_dataset_raw(churned_start_date=CHURNED_START_DATE,
                  churned_end_date=CHURNED_END_DATE,
                  inter_list=INTER_LIST,
                  raw_data_path='source/train/',
                  dataset_path='dataset/',
                  mode='train')

LOGGER.debug('Build test dataset')
build_dataset_raw(churned_start_date=CHURNED_START_DATE,
                  churned_end_date=CHURNED_END_DATE,
                  inter_list=INTER_LIST,
                  raw_data_path='source/test/',
                  dataset_path='dataset/',
                  mode='test')